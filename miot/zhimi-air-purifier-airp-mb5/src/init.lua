-- Xiaomi Smart Air Purifier 4 Driver
...
local fanModeFanMode = capabilities["concertmirror08464.zhimiAirAirpMb5FanMode"]
local fanSpeedFanSpeed = capabilities["concertmirror08464.zhimiAirAirpMb5FanSpeed"]
local deviceControlsBuzzer = capabilities["concertmirror08464.zhimiAirAirpMb5Buzzer"]
local deviceControlsChildLock = capabilities["concertmirror08464.zhimiAirAirpMb5ChildLock"]
local deviceControlsLedBrightness = capabilities["concertmirror08464.zhimiAirAirpMb5LedBrightness"]
...
-- MIoT model: zhimi.airp.mb5
-- Source: python-miio AirPurifierMiot mapping, python-miio issue #1406, openHAB model docs, MIoT spec zhimi-mb5 v1.
...
local BUZZER_SIID = 6
local BUZZER_PIID = 1
local LED_BRIGHTNESS_SIID = 13
local LED_BRIGHTNESS_PIID = 2
local CHILD_LOCK_SIID = 8
local CHILD_LOCK_PIID = 1
-- MIoT -> SmartThings LED brightness: 0 Close -> off, 1 Bright -> dim, 2 Brightest -> bright.
-- SmartThings -> MIoT uses the inverse table below.
local LED_BRIGHTNESS_TO_ST = {[0] = "off", [1] = "dim", [2] = "bright"}
local ST_TO_LED_BRIGHTNESS = {off = 0, dim = 1, bright = 2}
...
local function ensure_profile(device)
    if not device:supports_capability_by_id(fanModeFanMode.ID, "main") then
        device:try_update_metadata({profile = "zhimi-airp-mb5"})
    end
end
...
local driver = Driver("miot-air-purifier-airp-mb5", {
