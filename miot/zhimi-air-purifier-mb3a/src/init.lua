-- Zhimi Air Purifier MB3A Driver
...
local fanModeFanMode = capabilities["concertmirror08464.zhimiAirMb3aFanMode"]
local fanSpeedFanSpeed = capabilities["concertmirror08464.zhimiAirMb3aFanSpeed"]
local deviceControlsBuzzer = capabilities["concertmirror08464.zhimiAirMb3aBuzzer"]
local deviceControlsChildLock = capabilities["concertmirror08464.zhimiAirMb3aChildLock"]
local deviceControlsLedBrightness = capabilities["concertmirror08464.zhimiAirMb3aLedBrightness"]
...
-- MIoT spec: urn:miot-spec-v2:device:air-purifier:0000A007:zhimi-mb3a:1
local AIR_PURIFIER_SIID = 2   -- 공기청정기
local ENVIRONMENT_SIID = 3    -- 환경 센서
local FILTER_SIID = 4         -- 필터

-- MIoT 속성 ID - Air Purifier (siid=2)
local POWER_PIID = 2          -- 전원 on/off
local MODE_PIID = 5           -- 모드 (Auto=0, Sleep=1, Favorite=2, Manual=3)
local FAN_LEVEL_PIID = 4      -- 팬 레벨 (1, 2, 3)

-- MIoT 속성 ID - Environment (siid=3)
local HUMIDITY_PIID = 7       -- 습도
local PM25_PIID = 6           -- PM2.5
local TEMPERATURE_PIID = 8    -- 온도

-- MIoT 속성 ID - Filter (siid=4)
local FILTER_LIFE_PIID = 3    -- 필터 수명 레벨 (%)
...
local function ensure_profile(device)
    if not device:supports_capability_by_id(fanModeFanMode.ID, "main") then
        device:try_update_metadata({profile = "zhimi-mb3a"})
    end
end
...
local driver = Driver("miot-air-purifier-mb3a", {
