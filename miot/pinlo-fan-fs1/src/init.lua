...
local fanModeCap = capabilities["concertmirror08464.pinloFanFs1FanMode"]
local indicatorLightCap = capabilities["concertmirror08464.pinloFanFs1IndicatorLight"]
local buzzerCap = capabilities["concertmirror08464.pinloFanFs1Buzzer"]
local childLockCap = capabilities["concertmirror08464.pinloFanFs1ChildLock"]
local horizontalAngleCap = capabilities["concertmirror08464.pinloFanFs1HorizontalAngleV2"]

local fanSpeedPercent = capabilities["fanSpeedPercent"]

local POLLING_TIMER = "polling_timer"
local DEFAULT_POLLING_INTERVAL = 60
local PROFILE_NAME = "pinlo-fan-fs1"

-- MIoT model: pinlo.fan.fs1
-- Source: MIoT spec pinlo-fs1 v1.
-- Fan service (siid=2)
local FAN_SIID = 2
local POWER_PIID = 1
...
local INDICATOR_LIGHT_PIID = 1    -- RW screen on/off

-- Alarm service (siid=11)
local BUZZER_SIID = 11
...
local function level_to_percent(level)
    return LEVEL_TO_PERCENT[level] or 25
end

local function percent_to_level(percent)
    if percent <= 25 then return 0 end
    if percent <= 50 then return 1 end
    if percent <= 75 then return 2 end
    return 3
end
...
local function set_fan_speed_handler(_, device, command)
    ...
    local percent = math.max(0, math.min(100, command.args.percent))
    local level = percent_to_level(percent)
    local speed = level_to_percent(level)
...
local function set_fan_oscillation_mode_handler(_, device, command)
    ...
    local ok = pcall(miot.set, device, ip, token, FAN_SIID, HORIZONTAL_SWING_PIID, mode == "horizontal")
    if ok then
...
local function device_added(_, device)
    ...
    device:emit_event(capabilities.switch.switch.off())
    device:emit_event(fanSpeedPercent.percent({value = 25, unit = "%"}))
...
local driver = Driver("miot-pinlo-fan-fs1", {
