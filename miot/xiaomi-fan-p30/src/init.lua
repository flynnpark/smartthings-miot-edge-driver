-- Mi Smart Standing Fan 2 Driver
...
local fanModeCap = capabilities["concertmirror08464.xiaomiFanP30FanMode"]
local indicatorLightCap = capabilities["concertmirror08464.xiaomiFanP30IndicatorLight"]
local buzzerCap = capabilities["concertmirror08464.xiaomiFanP30Buzzer"]
local childLockCap = capabilities["concertmirror08464.xiaomiFanP30ChildLock"]
local horizontalAngleCap = capabilities["concertmirror08464.xiaomiFanP30HorizontalAngleV2"]

local fanSpeedPercent = capabilities["fanSpeedPercent"]

local POLLING_TIMER = "polling_timer"
local DEFAULT_POLLING_INTERVAL = 60
local PROFILE_NAME = "xiaomi-fan-p30"

-- MIoT model: xiaomi.fan.p30
-- Source: exact MIoT spec urn:miot-spec-v2:device:fan:0000A005:xiaomi-p30:1:0000D062.
-- Fan service (siid=2)
local FAN_SIID = 2
local POWER_PIID = 1
local FAULT_PIID = 2              -- fault status, not exposed
local MODE_PIID = 3               -- 0=Normal, 1=Nature
local FAN_LEVEL_PIID = 4          -- level bucket, not exposed
local SWING_MODE_PIID = 6         -- horizontal oscillation on/off
local FAN_SPEED_PIID = 5          -- 1..100
local SWING_ANGLE_PIID = 7        -- 30, 60, 90, 120, 140
...
local driver = Driver("miot-xiaomi-fan-p30", {
