-- Xiaomi Smart Fish Tank MYG100 Driver
...
local controlsIndicatorLight = capabilities["concertmirror08464.xiaomiTankM100IndicatorLight"]
local controlsPumpFlux = capabilities["concertmirror08464.xiaomiTankM100PumpFlux"]
local controlsFeederStatus = capabilities["concertmirror08464.xiaomiTankM100FeederStatus"]
local controlsPumpStatus = capabilities["concertmirror08464.xiaomiTankM100PumpStatus"]
local controlsFeedNow = capabilities["concertmirror08464.xiaomiTankM100FeedNow"]
local lightLightMode = capabilities["concertmirror08464.xiaomiTankM100LightMode"]
local lightFlowSpeed = capabilities["concertmirror08464.xiaomiTankM100FlowSpeed"]
local lightLightBrightness = capabilities["concertmirror08464.xiaomiTankM100LightBrightness"]
local lightLightSwitch = capabilities["concertmirror08464.xiaomiTankM100LightSwitch"]

local POLLING_TIMER = "polling_timer"
local DEFAULT_POLLING_INTERVAL = 60

-- MIoT model: hfjh.fishbowl.m100
-- specModel: hfjh-m100
...
--   piid=3 pump-flux, uint8, RW: 0=level1, 1=level2, 2=level3
--   piid=4 temperature, int8, R, 0..99 C
--   piid=5 feeding-measure, uint8, action input, 1..3
--   piid=6 water-pump-status, uint8, R: 0=notInserted, 1=closed, 2=opened, 3=low, 4=blocked, 5=fault
--   aiid=1 pet-food-out, action, in=[piid=5]
-- Filter services (siid=10 and siid=11 share the same contract)
...
-- Alarm service (siid=13)
--   piid=1 alarm/buzzer, bool, RW
-- Indicator light service (siid=14)
--   piid=1 on, bool, RW
--   piid=2 brightness, uint8, RW: 0=low, 1=middle, 2=high
-- Light service (siid=15)
--   piid=1 light on, bool, RW
--   piid=2 mode, uint8, RW: 0=day, 1=flow, 2=waterGrass, 3..6=color1..4, 7=nightLight, 8=color5
--   piid=3 brightness, uint8, RW, 1..100 %
--   piid=5 flow-speed-level, uint8, RW: 0=low, 1=medium, 2=high
-- Physical controls locked service (siid=16)
--   piid=1 child lock, bool, RW
-- Private light-custom service (siid=4)
--   piid=5 light-edit-color, uint32, RW, 1..16777215, exposed as colorControl
-- Private feeder-custom service (siid=6)
...
local TEMPERATURE_PIID = 4
local FEEDING_MEASURE_PIID = 5
local WATER_PUMP_STATUS_PIID = 6
local PET_FOOD_OUT_AIID = 1

local LIGHT_CUSTOM_SIID = 4
local LIGHT_EDIT_COLOR_PIID = 5

local FEEDER_CUSTOM_SIID = 6
...
local FLOW_SPEED_PIID = 5

local CHILD_LOCK_SIID = 16
        ...
        if result.code == 0 then
            ...
            elseif siid == INDICATOR_LIGHT_SIID then
                ...
                elseif piid == INDICATOR_BRIGHTNESS_PIID then
                    local brightness = INDICATOR_BRIGHTNESS_TO_ST[value]
                    if brightness then
            ...
            elseif siid == FEEDER_CUSTOM_SIID and piid == FEEDER_STATUS_PIID then
...
local function set_indicator_brightness_handler(_, device, command)
    ...
    local ok = pcall(miot.set, device, ip, token, INDICATOR_LIGHT_SIID, INDICATOR_BRIGHTNESS_PIID, value)
    if ok then
        device:emit_event(controlsIndicatorBrightness.indicatorBrightness({value = brightness}))
    end
end
...
local function ensure_profile(device)
    if not device:supports_capability_by_id(controlsIndicatorBrightness.ID, "main") then
...
local function device_added(_, device)
    ...
    device:emit_event(controlsPumpStatus.pumpStatus({value = "closed"}))
    ...
    device:emit_event(controlsIndicatorBrightness.indicatorBrightness({value = "middle"}))
...
local driver = Driver("miot-xiaomi-fish-tank-m100", {
