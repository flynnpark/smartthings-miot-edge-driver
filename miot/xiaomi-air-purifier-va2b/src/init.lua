-- Xiaomi Air Purifier VA2B Driver
...
local airPurifierModeCap = capabilities["concertmirror08464.xiaomiAirVa2bMode"]
local fanLevelCap = capabilities["concertmirror08464.xiaomiAirVa2bFanLevel"]
local anionCap = capabilities["concertmirror08464.xiaomiAirVa2bAnion"]
local buzzerCap = capabilities["concertmirror08464.xiaomiAirVa2bBuzzer"]
local childLockCap = capabilities["concertmirror08464.xiaomiAirVa2bChildLock"]
local displayLevelCap = capabilities["concertmirror08464.xiaomiAirVa2bDisplayLevel"]

local POLLING_TIMER = "polling_timer"
local DEFAULT_POLLING_INTERVAL = 60
local PROFILE_NAME = "xiaomi-air-purifier-va2b"

-- MIoT model: xiaomi.airp.va2b
...
--   piid=2 fault, uint8, R; 0=No Faults, 1=Particle Sensor Fault, 2=Temp Sensor Fault, 3=Hum Sensor Fault, 4=No Filter: not exposed
--   piid=4 mode, uint8, RW; 0=Auto, 3=Sleep, 5=Favorite, 6=None -> xiaomiAirVa2bMode.airPurifierMode
--   piid=5 fan-level, uint8, RW; 0=Level1, 1=Level2, 2=Level3 -> xiaomiAirVa2bFanLevel.fanLevel
--   piid=6 anion, bool, RW -> xiaomiAirVa2bAnion.anion
-- Environment service (siid=3)
--   piid=1 relative-humidity, uint8, R; range 0..100 step 1 percentage -> relativeHumidityMeasurement
--   piid=2 temperature, float, R; range -30..100 step 1 celsius -> temperatureMeasurement
...
--   piid=2 filter-used-time, uint16, R; range 0..5000 step 1 hours: not exposed
--   piid=3 filter-left-time, uint16, R; range 0..5000 step 1 hours: not exposed
-- Alarm service (siid=6)
--   piid=1 alarm, bool, RW -> xiaomiAirVa2bBuzzer.buzzer
-- Physical Control Locked service (siid=8)
--   piid=1 physical-controls-locked, bool, RW -> xiaomiAirVa2bChildLock.childLock
-- custom-service service (siid=9)
--   piid=1 moto-speed-rpm, uint16, R; range 0..65535 step 1: not exposed
--   piid=3 filter-debug-time, uint16, W; range 0..5000 step 1 hours: not exposed
--   piid=4 aqi-update-heartbeat, uint16, RW; range 0..65535 step 1 seconds: not exposed
--   piid=6 favorite-square, string, R: not exposed
...
--   piid=6 motor-speed-rpm, uint16, R; range 0..65535 step 1: not exposed
--   piid=7 aqi-update-heartbeat, uint16, RW; range 0..65535 step 1 seconds: not exposed
--   piid=8 favorite-square, string, R: not exposed
-- Screen service (siid=13)
--   piid=1 brightness, uint8, RW; 0=Normal, 1=Dim, 2=Close -> xiaomiAirVa2bDisplayLevel.displayLevel
-- Air Purifier Favorite service (siid=14)
--   piid=1 fan-level, uint8, RW; 0=Level0, 1=Level1, 2=Level2, 3=Level3, 4=Level4, 5=Level5, 6=Level6, 7=Level7, 8=Level8, 9=Level9, 10=Level10, 11=Level11: not exposed
...
local function ensure_profile(device)
    if not device:supports_capability_by_id("concertmirror08464.xiaomiAirVa2bMode", "main") then
...
local driver = Driver("miot-xiaomi-air-purifier-va2b", {
