-- Mijia DC Inverter Tower Fan 2 Driver
...
local fanModeCap = capabilities["concertmirror08464.dmakerFanP45FanMode"]
local indicatorLightCap = capabilities["concertmirror08464.dmakerFanP45IndicatorLight"]
local buzzerCap = capabilities["concertmirror08464.dmakerFanP45Buzzer"]
local childLockCap = capabilities["concertmirror08464.dmakerFanP45ChildLock"]
local horizontalAngleCap = capabilities["concertmirror08464.dmakerFanP45HorizontalAngleV2"]

local fanSpeedPercent = capabilities["fanSpeedPercent"]

local POLLING_TIMER = "polling_timer"
local DEFAULT_POLLING_INTERVAL = 60
local PROFILE_NAME = "dmaker-fan-p45"

-- MIoT model: dmaker.fan.p45
-- Source: python-miio FanMiot exact model, MIoT spec dmaker-p45 v1, and model docs.
-- Fan service (siid=2)
local FAN_SIID = 2
local POWER_PIID = 1
local MODE_PIID = 3               -- RW 0=Straight, 1=Natural, 2=Sleep
...
local SWING_ANGLE_PIID = 5        -- RW 30, 60, 90, 120, 150; not exposed

-- Fan actions (siid=2), not exposed because switch and oscillation controls cover core use.
local TOGGLE_ACTION_IID = 1
local TURN_LEFT_ACTION_IID = 2
local TURN_RIGHT_ACTION_IID = 3
...
-- Dmaker service (siid=8), stepless speed is core; movement and natural wind customization are not exposed.
local DM_SERVICE_SIID = 8
local FAN_SPEED_PIID = 1          -- RW stepless fan level 1..100
local START_LEFT_PIID = 3         -- W start left, not exposed
local START_RIGHT_PIID = 4        -- W start right, not exposed
...
local driver = Driver("miot-dmaker-fan-p45", {
