...
-- Exact local MIoT model: Xiaomi Air Purifier UA3
-- Property mappings below come from the exact MIoT specification.
--
--   siid=2 piid=1 -> power
--   siid=2 piid=4 -> xiaomiAirUa3Mode.airPurifierMode
--   siid=2 piid=5 -> xiaomiAirUa3FanLevel.fanLevel
--   siid=2 piid=7 -> xiaomiAirUa3Uv.uv
--   siid=2 piid=9 -> xiaomiAirUa3Plasma.plasma
--   siid=3 piid=1 -> humidity
--   siid=3 piid=7 -> temperature
--   siid=3 piid=4 -> pm25
--   siid=3 piid=5 -> pm10
--   siid=3 piid=8 -> co2
--   siid=3 piid=10 -> formaldehyde
--   siid=4 piid=1 -> filter
--   siid=6 piid=1 -> xiaomiAirUa3Buzzer.buzzer
--   siid=7 piid=1 -> xiaomiAirUa3Display.display
--   siid=7 piid=2 -> xiaomiUa3AutoBrightness.autoDisplayBrightness
--   siid=8 piid=1 -> xiaomiAirUa3ChildLock.childLock
...
local function set_autoDisplayBrightness_handler(_, device, command)
    local ip, token = get_device_config(device)
    if not ip then return end

    local requested = command.args.autoDisplayBrightness
    local ok = pcall(miot.set, device, ip, token, 7, 2, requested == "on")
    if ok then
        device:emit_event(autoDisplayBrightnessCap.autoDisplayBrightness({value = requested}))
    end
end
