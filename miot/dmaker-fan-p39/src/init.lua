...
local fanModeCap = capabilities["concertmirror08464.dmakerFanP39FanMode"]
local indicatorLightCap = capabilities["concertmirror08464.dmakerFanP39IndicatorLight"]
local buzzerCap = capabilities["concertmirror08464.dmakerFanP39Buzzer"]
local childLockCap = capabilities["concertmirror08464.dmakerFanP39ChildLock"]
local horizontalAngleCap = capabilities["concertmirror08464.dmakerFanP39HorizontalAngleV2"]

local fanSpeedPercent = capabilities["fanSpeedPercent"]

local POLLING_TIMER = "polling_timer"
local DEFAULT_POLLING_INTERVAL = 60
local PROFILE_NAME = "dmaker-fan-p39"

-- MIoT model: dmaker.fan.p39
-- Source: python-miio FanMiot exact model, MIoT spec dmaker-p39 v1, and model docs.
-- Fan service (siid=2)
local FAN_SIID = 2
local POWER_PIID = 1              -- RW on/off
...
local SET_MOVE_PIID = 10          -- W 0=none, 1=left, 2=right; not exposed
local FAN_SPEED_PIID = 11         -- RW stepless fan level 1..100

-- Fan action (siid=2), not exposed because switch covers core use.
...
-- MIoT -> SmartThings fanMode:
-- 0 Straight Wind -> normal, 1 Natural Wind -> nature, 2 Sleep -> sleep
-- SmartThings -> MIoT uses the inverse table below.
local MODE_TO_ST = {
    [0] = "normal",
    [1] = "nature",
    [2] = "sleep"
...
local driver = Driver("miot-dmaker-fan-p39", {
