-- Mijia Smart Air Purifier MAX Driver
...
local controls = capabilities["concertmirror08464.airPurifierMaxControls"]

local POLLING_TIMER = "polling_timer"
local DEFAULT_POLLING_INTERVAL = 60

-- MIoT model: xiaomi.airp.mb6
-- Source: Xiaomi product docs, hass-xiaomi-miot LAN MIoT support, model docs,
-- and MIoT spec xiaomi-mb6 v1.
...
local HCHO_PIID = 6              -- R float, 0..1 mg/m3
...
local SCREEN_BRIGHTNESS_PIID = 2 -- RW enum: 0 dim, 1 normal, 2 half
...
local function ensure_profile(device)
    if not device:supports_capability_by_id(controls.ID, "main") then
        device:try_update_metadata({profile = "xiaomi-air-purifier-mb6"})
    end
end
...
local driver = Driver("miot-air-purifier-mb6", {
