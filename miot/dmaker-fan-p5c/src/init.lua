-- Mijia Smart DC Standing Fan 1X Driver
...
local fanModeCap = capabilities["concertmirror08464.dmakerFanP5cFanMode"]
local indicatorLightCap = capabilities["concertmirror08464.dmakerFanP5cIndicatorLight"]
local buzzerCap = capabilities["concertmirror08464.dmakerFanP5cBuzzer"]
local childLockCap = capabilities["concertmirror08464.dmakerFanP5cChildLock"]
local horizontalAngleCap = capabilities["concertmirror08464.dmakerFanP5cHorizontalAngleV2"]

local fanSpeedPercent = capabilities["fanSpeedPercent"]

local POLLING_TIMER = "polling_timer"
local DEFAULT_POLLING_INTERVAL = 60
local PROFILE_NAME = "dmaker-fan-p5c"

-- MIoT model: dmaker.fan.p5c
-- Source: exact MIoT spec urn:miot-spec-v2:device:fan:0000A005:dmaker-p5c:2
-- Fan service (siid=2)
local FAN_SIID = 2
local POWER_PIID = 1
...
local FAN_STATUS_PIID = 6         -- supply status; not exposed
...
local function poll_device_status(device)
    ...
    local properties = {
        ...
        {siid = FAN_SIID, piid = FAN_STATUS_PIID},
...
local driver = Driver("miot-dmaker-fan-p5c", {
