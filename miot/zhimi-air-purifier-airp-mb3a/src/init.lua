-- Zhimi Air Purifier AIRP MB3A Driver
...
local fanModeFanMode = capabilities["concertmirror08464.zhimiAirAirpMb3aFanMode"]
...
local function ensure_profile(device)
    if not device:supports_capability_by_id(fanModeFanMode.ID, "main") then
        device:try_update_metadata({profile = "zhimi-airp-mb3a"})
    end
end
...
local driver = Driver("miot-air-purifier-airp-mb3a", {
