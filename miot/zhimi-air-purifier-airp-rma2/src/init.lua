...
-- Exact local MIoT model: Zhimi Air Purifier AIRP RMA2
-- Property mappings below come from the exact MIoT specification.
--
--   siid=2 piid=1 -> power
--   siid=2 piid=4 -> zhimiAirAirpRma2Mode.airPurifierMode
--   siid=3 piid=1 -> humidity
--   siid=3 piid=7 -> temperature
--   siid=3 piid=4 -> pm25
--   siid=4 piid=1 -> filter
--   siid=5 piid=1 -> zhimiAirAirpRma2Buzzer.buzzer
--   siid=6 piid=1 -> zhimiAirAirpRma2ChildLock.childLock
--   siid=7 piid=2 -> zhimiAirAirpRma2DisplayLevel.displayLevel
--   siid=11 piid=1 -> zhimiAirAirpRma2FanLevel.fanLevel
...
local function set_fanLevel_handler(_, device, command)
    ...
    local ok = pcall(miot.set, device, ip, token, 11, 1, value)
    if ok then
...
local function set_buzzer_handler(_, device, command)
    ...
    local ok = pcall(miot.set, device, ip, token, 5, 1, requested == "on")
    if ok then
