-- Xiaomi Air Conditioner R09 Driver
...
local humidityCap = capabilities["concertmirror08464.xiaomiAcR09Humidity"]
local vSwingCap = capabilities["concertmirror08464.xiaomiAcR09VSwing"]
local vaneCap = capabilities["concertmirror08464.xiaomiAcR09Vane"]
local ecoCap = capabilities["concertmirror08464.xiaomiAcR09Eco"]
local heaterCap = capabilities["concertmirror08464.xiaomiAcR09Heater"]
local sleepCap = capabilities["concertmirror08464.xiaomiAcR09Sleep"]
local dryerCap = capabilities["concertmirror08464.xiaomiAcR09Dryer"]
local softWindCap = capabilities["concertmirror08464.xiaomiAcR09SoftWind"]
local filterCap = capabilities["concertmirror08464.xiaomiAcR09Filter"]
local alarmCap = capabilities["concertmirror08464.xiaomiAcR09Alarm"]
local indicatorCap = capabilities["concertmirror08464.xiaomiAcR09Indicator"]

local POLLING_TIMER = "polling_timer"
local DEFAULT_POLLING_INTERVAL = 60
local PROFILE_NAME = "xiaomi-air-conditioner-r09h00"

-- MIoT model: xiaomi.airc.r09h00
...
--   piid=3 fault, string, R, vendor code, not exposed
...
--   piid=19/20 favorite on and type: cloud comfort presets, not exposed
-- Fan Control service (siid=3)
--   piid=2 fan-level, uint8, RW enum: 0=auto, 1..8 levels
--   piid=4 vertical-swing, bool, RW
--   piid=6 vertical-angle, uint8, RW enum: 0=off, 1=top rating, 2=top,
--     3=middle, 4=bottom, 5=lower rating, 6=surround, 7=upper, 8=lower
...
-- Filter service (siid=21)
--   piid=1 filter-life-level, uint8, R range 0..101, percent
--   aiid=1 reset-filter-life
--
-- Not exposed: siid=22 repeats the same filter contract for the second element,
-- power-consumption and electricity are cumulative counters, the
-- air-conditioner-dev-mode, machine-state, flag-bit, system-parm, and
-- favorite-type-data services expose factory tuning or diagnostics, the enhance
-- service duplicates the fan level and carries schedule blobs, and
-- mosquito-repellent applies to a consumable accessory.
...
local SUPPORTED_FAN_MODES = {
    "auto", "levelOne", "levelTwo", "levelThree", "levelFour",
    ...
    for _, result in ipairs(response.result) do
        if result.code == 0 then
            ...
            elseif siid == FILTER_SIID and piid == FILTER_LIFE_PIID then
                -- The device reports 0..101, so clamp the spare step to 100.
                local level = math.min(100, value)
                device:emit_event(filterCap.filterLife({value = level, unit = "%"}))
...
local function device_added(_, device)
    ...
    device:emit_event(filterCap.filterLife({value = 100, unit = "%"}))
    device:emit_event(alarmCap.alarm({value = "off"}))
    device:emit_event(indicatorCap.indicatorBrightness({value = "auto"}))
end
...
local driver = Driver("miot-xiaomi-air-conditioner-r09h00", {
