-- Xiaomi Dehumidifier Lite Driver
...
local statusTankStatus = capabilities["concertmirror08464.xiaomiDehumLiteTankStatus"]
local statusFilterStatus = capabilities["concertmirror08464.xiaomiDehumLiteFilterStatus"]
local statusFault = capabilities["concertmirror08464.xiaomiDehumLiteFault"]

local POLLING_TIMER = "polling_timer"
local DEFAULT_POLLING_INTERVAL = 60

-- MIoT model: xiaomi.derh.lite
...
--   piid=2 fault, uint8, R: 0=noFaults, 1=waterFull, 2=tempHumError,
--     3=copperPipeTempError, 4=communicationFailure, 5=filterClean,
--     6=defrost, 7=motorStuck, 8=overloadProtect, 9=lackOfRefrigerant
...
--   piid=1 on, bool, RW
--   piid=2 mode, uint8, RW: 0=off, 1=dim, 2=bright
...
local FAULT_TO_ST = {
    ...
    [1] = "waterFull",
    [2] = "tempHumError",
    [3] = "copperPipeTempError",
    [4] = "communicationFailure",
    [5] = "filterClean",
    [6] = "defrost",
    [7] = "motorStuck",
    [8] = "overloadProtect",
...
local function ensure_profile(device)
    if not device:supports_capability_by_id(controlsMode.ID, "main") then
        device:try_update_metadata({profile = "xiaomi-dehumidifier-lite"})
    end
end
...
local driver = Driver("miot-xiaomi-dehumidifier-lite", {
