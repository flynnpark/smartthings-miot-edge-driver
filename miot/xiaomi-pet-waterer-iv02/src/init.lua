-- Xiaomi Pet Waterer IV02 Driver
...
local modeControl = capabilities["concertmirror08464.xiaomiPetWaterIv02Mode"]
local intervalControl = capabilities["concertmirror08464.xiaomiPetWaterIv02Interval"]
local operatingStatus = capabilities["concertmirror08464.xiaomiPetWaterIv02Status"]
local waterShortageStatus = capabilities["concertmirror08464.xiaomiPetWaterIv02WaterShortage"]
local filterLifeStatus = capabilities["concertmirror08464.xiaomiPetWaterIv02FilterLife"]
local pumpBlockStatus = capabilities["concertmirror08464.xiaomiPetWaterIv02PumpBlock"]
local childLockControl = capabilities["concertmirror08464.xiaomiPetWaterIv02ChildLock"]
local noDisturbControl = capabilities["concertmirror08464.xiaomiPetWaterIv02NoDisturb"]

local POLLING_TIMER = "polling_timer"
local DEFAULT_POLLING_INTERVAL = 60
local EXPECTED_PROFILE_NAME = "xiaomi-pet-waterer-iv02"

-- MIoT model: xiaomi.pet_waterer.iv02
...
--   piid=7 out-water-interval, uint8, RW range 0..120 step 15, not exposed
...
local function poll_device_status(device)
    ...
    local properties = {
        {siid = FOUNTAIN_SIID, piid = POWER_PIID},
...
local function switch_on_handler(_, device, _)
    ...
    local ok = pcall(miot.set, device, ip, token, FOUNTAIN_SIID, POWER_PIID, true)
    if ok then
...
local function switch_off_handler(_, device, _)
    ...
    local ok = pcall(miot.set, device, ip, token, FOUNTAIN_SIID, POWER_PIID, false)
    if ok then
...
local function set_mode_handler(_, device, command)
    ...
    pcall(miot.set, device, ip, token, FOUNTAIN_SIID, POWER_PIID, true)
...
local driver = Driver("miot-xiaomi-pet-waterer-iv02", {
