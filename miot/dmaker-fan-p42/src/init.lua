...
local fanModeCap = capabilities["concertmirror08464.dmakerFanP42FanMode"]
local indicatorLightCap = capabilities["concertmirror08464.dmakerFanP42IndicatorLight"]
local buzzerCap = capabilities["concertmirror08464.dmakerFanP42Buzzer"]
local childLockCap = capabilities["concertmirror08464.dmakerFanP42ChildLock"]
local horizontalAngleCap = capabilities["concertmirror08464.dmakerFanP42HorizontalAngleV2"]

local fanSpeedPercent = capabilities["fanSpeedPercent"]

local POLLING_TIMER = "polling_timer"
local DEFAULT_POLLING_INTERVAL = 60
local PROFILE_NAME = "dmaker-fan-p42"

-- MIoT model: dmaker.fan.p42
-- Source: exact MIoT spec urn:miot-spec-v2:device:fan:0000A005:dmaker-p42:1.
-- Fan service (siid=2)
local FAN_SIID = 2
local POWER_PIID = 1
local GEAR_FAN_LEVEL_PIID = 2     -- RW level bucket 1..4, not exposed separately
local MODE_PIID = 3               -- 0=Straight Wind, 1=Natural Wind
local SWING_MODE_PIID = 4         -- horizontal oscillation on/off
local SWING_ANGLE_PIID = 5        -- 30, 60, 90, 120, 140
local INDICATOR_LIGHT_PIID = 7    -- brightness on/off
local BUZZER_PIID = 8
local FAN_SPEED_PIID = 10         -- 1..100

-- Auxiliary services
local INDICATOR_LIGHT_SIID = FAN_SIID
local BUZZER_SIID = FAN_SIID
...
local driver = Driver("miot-dmaker-fan-p42", {
