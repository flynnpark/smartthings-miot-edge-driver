-- Mijia Smart Air Purifier 6 Driver
...
local controls = capabilities["concertmirror08464.airPurifier6Controls"]

local POLLING_TIMER = "polling_timer"
local DEFAULT_POLLING_INTERVAL = 60

-- MIoT model: xiaomi.airp.mb5
-- Source: hass-xiaomi-miot LAN MIoT support, model docs, MIoT spec xiaomi-mb5 v1.
-- Air purifier service (siid=2)
local AIR_PURIFIER_SIID = 2
local POWER_PIID = 1             -- RW bool
local FAULT_PIID = 2             -- R enum, diagnostic only
local MODE_PIID = 4              -- RW enum: 0 auto, 3 sleep, 5 favorite, 6 none
local FAN_LEVEL_PIID = 5         -- RW enum: 0 level1, 1 level2, 2 level3
local ANION_PIID = 6             -- RW bool
...
local PM25_PIID = 4              -- R uint16, 0..1000 ug/m3
local PM10_PIID = 5              -- R uint16, 0..100 ug/m3
local PM1_PIID = 9               -- R uint16, 0..1000 ug/m3, not exposed
...
-- Favorite service (siid=9), filter debug/tag, AQI heartbeat, diagnostics, and
-- custom services are intentionally not exposed as core SmartThings controls.
        ...
        if result.code == 0 then
            ...
            if siid == AIR_PURIFIER_SIID then
                ...
                elseif piid == ANION_PIID then
                    device:emit_event(controls.anion({value = bool_to_on_off(value)}))
...
local function handle_set_anion(_, device, command)
    local ip, token = get_device_config(device)
    if not ip then return end

    local anion = command.args.anion
    local value = anion == "on"
    local ok = pcall(miot.set, device, ip, token, AIR_PURIFIER_SIID, ANION_PIID, value)
    if ok then
        device:emit_event(controls.anion({value = anion}))
    end
end
...
local function device_added(_, device)
    ...
    device:emit_event(controls.anion({value = "off"}))
...
local driver = Driver("miot-xiaomi-air-purifier-mb5", {
    ...
    capability_handlers = {
        ...
        [controls.ID] = {
            ...
            [controls.commands.setAnion.NAME] = handle_set_anion,
