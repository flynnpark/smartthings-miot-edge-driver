...
local airModeAirPurifierMode = capabilities["concertmirror08464.zhimiAirV1AirPurifierMode"]
local deviceControlsBuzzer = capabilities["concertmirror08464.zhimiAirV1Buzzer"]
local deviceControlsChildLock = capabilities["concertmirror08464.zhimiAirV1ChildLock"]
local deviceControlsLedBrightness = capabilities["concertmirror08464.zhimiAirV1LedBrightness"]

local POLLING_TIMER = "polling_timer"
local DEFAULT_POLLING_INTERVAL = 60

-- miIO model: zhimi.airpurifier.v1
...
local function ensure_profile(device)
    if not device:supports_capability_by_id(airModeAirPurifierMode.ID, "main") then
        device:try_update_metadata({profile = "zhimi-v1"})
    end
end
...
local driver = Driver("miio-air-purifier-v1", {
