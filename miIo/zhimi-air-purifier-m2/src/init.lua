...
local airModeAirPurifierMode = capabilities["concertmirror08464.zhimiAirM2AirPurifierMode"]
local deviceControlsBuzzer = capabilities["concertmirror08464.zhimiAirM2Buzzer"]
local deviceControlsChildLock = capabilities["concertmirror08464.zhimiAirM2ChildLock"]
local deviceControlsLedBrightness = capabilities["concertmirror08464.zhimiAirM2LedBrightness"]

local POLLING_TIMER = "polling_timer"
local DEFAULT_POLLING_INTERVAL = 60

-- miIO model: zhimi.airpurifier.m2
...
local SUPPORTED_MODES = {"auto", "sleep", "favorite", "idle"}
...
local function ensure_profile(device)
    if not device:supports_capability_by_id(airModeAirPurifierMode.ID, "main") then
        device:try_update_metadata({profile = "zhimi-m2"})
    end
end
...
local driver = Driver("miio-air-purifier-m2", {
