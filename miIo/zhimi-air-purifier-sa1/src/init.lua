...
local airModeAirPurifierMode = capabilities["concertmirror08464.zhimiAirSa1AirPurifierMode"]
local deviceControlsBuzzer = capabilities["concertmirror08464.zhimiAirSa1Buzzer"]
local deviceControlsChildLock = capabilities["concertmirror08464.zhimiAirSa1ChildLock"]
local deviceControlsLedBrightness = capabilities["concertmirror08464.zhimiAirSa1LedBrightness"]

local POLLING_TIMER = "polling_timer"
local DEFAULT_POLLING_INTERVAL = 60

-- miIO model: zhimi.airpurifier.sa1
...
local function ensure_profile(device)
    if not device:supports_capability_by_id(airModeAirPurifierMode.ID, "main") then
        device:try_update_metadata({profile = "zhimi-sa1"})
    end
end
...
local driver = Driver("miio-air-purifier-sa1", {
