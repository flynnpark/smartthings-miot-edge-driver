...
local humidifierCoreTargetHumidity = capabilities["concertmirror08464.zhimiHumCb1TargetHumidity"]
local humidifierCoreFanMode = capabilities["concertmirror08464.zhimiHumCb1FanMode"]
local humidifierWaterDryMode = capabilities["concertmirror08464.zhimiHumCb1DryMode"]
local humidifierWaterWaterLevel = capabilities["concertmirror08464.zhimiHumCb1WaterLevel"]
local deviceControlsBuzzer = capabilities["concertmirror08464.zhimiHumCb1Buzzer"]
local deviceControlsChildLock = capabilities["concertmirror08464.zhimiHumCb1ChildLock"]
local deviceControlsLedBrightness = capabilities["concertmirror08464.zhimiHumCb1LedBrightness"]

local POLLING_TIMER = "polling_timer"
local DEFAULT_POLLING_INTERVAL = 60

-- miIO model: zhimi.humidifier.cb1
...
-- python-miio notes CB1 is limited to one get_prop item per request.
...
local function poll_device_status(device)
    ...
    if values.temperature and type(values.temperature) == "number" then
        device:emit_event(capabilities.temperatureMeasurement.temperature({value = values.temperature, unit = "C"}))
    end
    ...
    if water_level then
        device:emit_event(humidifierWaterWaterLevel.waterLevel({value = water_level, unit = "%"}))
    end

    local dry = miio_on_off_to_st(values.dry)
    if dry then
        device:emit_event(humidifierWaterDryMode.dryMode({value = dry}))
    end
...
local function handle_set_dry_mode(_, device, command)
    ...
    if miio.set_prop(device, ip, token, "set_dry", {mode}) then
        device:emit_event(humidifierWaterDryMode.dryMode({value = mode}))
    end
end
...
local function ensure_profile(device)
    if not device:supports_capability_by_id(humidifierCoreTargetHumidity.ID, "main") then
        device:try_update_metadata({profile = "zhimi-humidifier-cb1"})
    end
end
...
local function device_added(_, device)
    ...
    device:emit_event(humidifierWaterDryMode.dryMode({value = "off"}))
    device:emit_event(humidifierWaterWaterLevel.waterLevel({value = 0, unit = "%"}))
...
local driver = Driver("miio-humidifier-cb1", {
    ...
    capability_handlers = {
        ...
        [humidifierWaterDryMode.ID] = {
            [humidifierWaterDryMode.commands.setDryMode.NAME] = handle_set_dry_mode
        },
