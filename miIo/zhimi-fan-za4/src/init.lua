...
-- miIO model: zhimi.fan.za4
-- Source: current python-miio miio.integrations.zhimi.fan.fan classic Fan(Device).
-- Read properties use one-property get_prop requests for ZA4: angle, power,
--                  angle_enable, speed_level, natural_level, child_lock,
--                  buzzer, led_b
-- Write methods: set_power, set_speed_level, set_natural_level, set_angle_enable,
--                set_angle, set_led_b, set_buzzer, set_child_lock
...
local function poll_device_status(device)
    ...
    if type(values.led_b) == "number" then
        local brightness = DISPLAY_BRIGHTNESS_TO_ST[values.led_b]
        if brightness then
...
local driver = Driver("miio-zhimi-fan-za4", {
