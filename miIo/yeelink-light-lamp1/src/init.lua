...
local COLOR_TEMPERATURE_MIN = 2700
local COLOR_TEMPERATURE_MAX = 6500

-- miIO model: yeelink.light.lamp1
-- Core read properties: power, bright, ct
-- Core write methods: set_power, set_bright, set_ct_abx
...
local function get_properties(device, ip, token)
    local response = miio.cmd(device, ip, token, "get_prop", {"power", "bright", "ct"})
...
local driver = Driver("miio-yeelink-light-lamp1", {
