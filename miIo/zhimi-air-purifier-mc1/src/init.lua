...
local airModeAirPurifierMode = capabilities["concertmirror08464.zhimiAirMc1AirPurifierMode"]
local deviceControlsBuzzer = capabilities["concertmirror08464.zhimiAirMc1Buzzer"]
local deviceControlsChildLock = capabilities["concertmirror08464.zhimiAirMc1ChildLock"]
local deviceControlsLedBrightness = capabilities["concertmirror08464.zhimiAirMc1LedBrightness"]

local POLLING_TIMER = "polling_timer"
local DEFAULT_POLLING_INTERVAL = 60

-- miIO model: zhimi.airpurifier.mc1
...
local function ensure_profile(device)
    if not device:supports_capability_by_id(airModeAirPurifierMode.ID, "main") then
        device:try_update_metadata({profile = "zhimi-mc1"})
    end
end
...
local driver = Driver("miio-air-purifier-mc1", {
