...
local modeCap = capabilities["concertmirror08464.airdogAirX5Mode"]
local fanLevelCap = capabilities["concertmirror08464.airdogAirX5FanLevel"]
local childLockCap = capabilities["concertmirror08464.airdogAirX5ChildLock"]
local cleanFilterCap = capabilities["concertmirror08464.airdogAirX5CleanFilter"]

local POLLING_TIMER = "polling_timer"
local DEFAULT_POLLING_INTERVAL = 60
local PROFILE_NAME = "airdog-air-purifier-x5"
local CURRENT_MODE = "current_mode"
local CURRENT_FAN_LEVEL = "current_fan_level"

-- miIO model: airdog.airpurifier.x5
-- Source: python-miio miio.integrations.airdog.airpurifier.airpurifier_airdog,
-- class AirDogX3(Device), shared AVAILABLE_PROPERTIES entry.
-- Read properties via get_prop: power, mode, speed, lock, clean, pm
-- Write methods:
--   set_power [0|1]
--   set_wind [modeIndex, speed]  modeIndex 0=auto, 1=manual, 2=sleep
--   set_lock [0|1]
--   set_clean (no argument, marks the filter as cleaned)
-- Mode auto and sleep always send speed 1. Manual speed range for x5 is 1..5.

local PROPERTIES = {"power", "mode", "speed", "lock", "clean", "pm"}
...
local driver = Driver("miio-airdog-airpurifier-x5", {
