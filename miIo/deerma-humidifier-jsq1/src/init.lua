...
local modeCap = capabilities["concertmirror08464.deermaHumJsq1Mode"]
local targetHumidityCap = capabilities["concertmirror08464.deermaHumJsq1TargetHumidity"]
local indicatorLightCap = capabilities["concertmirror08464.deermaHumJsq1IndicatorLight"]
local buzzerCap = capabilities["concertmirror08464.deermaHumJsq1Buzzer"]
local waterShortageCap = capabilities["concertmirror08464.deermaHumJsq1WaterShortage"]
local tankAttachedCap = capabilities["concertmirror08464.deermaHumJsq1TankAttached"]
local wetProtectionCap = capabilities["concertmirror08464.deermaHumJsq1WetProtection"]

local POLLING_TIMER = "polling_timer"
local DEFAULT_POLLING_INTERVAL = 60
local PROFILE_NAME = "deerma-humidifier-jsq1"

-- miIO model: deerma.humidifier.jsq1
...
--                  waterstatus, watertankstatus, wet_and_protect
-- Write methods: Set_OnOff [0|1], Set_HumidifierGears [1..5],
--                SetLedState [0|1], SetTipSound_Status [0|1],
--                Set_HumiValue [0..99], Set_wet_and_protect [0|1]
...
local function poll_device_status(device)
    ...
    if type(values.wet_and_protect) == "number" then
        device:emit_event(wetProtectionCap.wetProtection({value = values.wet_and_protect == 1 and "on" or "off"}))
    end
end
...
local function set_wet_protection_handler(_, device, command)
    local ip, token = get_device_config(device)
    if not ip then return end

    local protection = command.args.wetProtection
    if miio.set_prop(device, ip, token, "Set_wet_and_protect", {protection == "on" and 1 or 0}) then
        device:emit_event(wetProtectionCap.wetProtection({value = protection}))
    end
end
...
local function device_added(_, device)
    ...
    device:emit_event(wetProtectionCap.wetProtection({value = "off"}))
end
...
local driver = Driver("miio-deerma-humidifier-jsq1", {
    ...
    capability_handlers = {
        ...
        [wetProtectionCap.ID] = {
            [wetProtectionCap.commands.setWetProtection.NAME] = set_wet_protection_handler
        },
