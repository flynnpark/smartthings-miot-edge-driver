...
local airModeAirPurifierMode = capabilities["concertmirror08464.zhimiAirV2AirPurifierMode"]
local deviceControlsBuzzer = capabilities["concertmirror08464.zhimiAirV2Buzzer"]
local deviceControlsChildLock = capabilities["concertmirror08464.zhimiAirV2ChildLock"]
local deviceControlsLedBrightness = capabilities["concertmirror08464.zhimiAirV2LedBrightness"]

local POLLING_TIMER = "polling_timer"
local DEFAULT_POLLING_INTERVAL = 60

-- miIO model: zhimi.airpurifier.v2
...
local function ensure_profile(device)
    if not device:supports_capability_by_id(airModeAirPurifierMode.ID, "main") then
        device:try_update_metadata({profile = "zhimi-v2"})
    end
end
...
local driver = Driver("miio-air-purifier-v2", {
