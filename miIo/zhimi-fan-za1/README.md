...
- `fanSpeedPercent`
- `fanOscillationMode`
- `concertmirror08464.zhimiFanZa1Controls`
- `refresh`
...
| Natural speed percent | RW | `natural_level`, `set_natural_level`, `1..100`; `0` disables natural mode | `fanSpeedPercent` in nature mode and `zhimiFanZa1Controls.fanMode` |
| Horizontal oscillation | RW | `angle_enable`, `set_angle_enable` with `on` / `off` | `fanOscillationMode` |
| LED brightness | RW | `led_b`, `set_led_b`; `0=bright`, `1=dim`, `2=off` | `zhimiFanZa1Controls.ledBrightness` |
| Buzzer | RW | `buzzer`, `set_buzzer`; `2=on`, `0=off` | `zhimiFanZa1Controls.buzzer` |
| Child lock | RW | `child_lock`, `set_child_lock` with `on` / `off` | `zhimiFanZa1Controls.childLock` |

Angle control: `zhimiFanZa1Controls.horizontalAngle` maps miIO `angle` / `set_angle`, `0..120`.
